...
export default function ViewDataset() {
    ...
    useEffect(() => {
        axios.get(`https://zensho.herokuapp.com/api/dataset/${id}`).then(
            (response) => {
    ...
    useEffect(() => {
        axios.get(`https://zensho.herokuapp.com/api/comment/${id}`).then(
            (response) => {
    ...
    const postComment = () => {
        ...
        axios.post("https://zensho.herokuapp.com/api/comment",data).then(
            response => {
                setCommentList([...commentList, {"username": username, "comment_body": commentBody, "dataset_id": id}])
                console.log(response)
                setCommentBody("")
            }
        )
        ...
        return (
            <div className="ml-64 page-width h-screen text-white flex flex-col items-center overflow-y-scroll background-gray">
                ...
                <div className="relative h-1/3 w-4/5 border-2 border-white mt-8 flex flex-col justify-end pl-8 pb-4 flex-shrink-0">
                    <img className="absolute -z-50 top-0 left-0 w-full h-full object-cover" src={data.imageSrc} alt="dataset"/>
                    <h1 className="z-0 font-bold text-3xl mb-2">{data.name}</h1>
                    <h2 className="z-0 font-extralight text-sm mb-2">{data.description}</h2>
                    <div className="z-0 flex flex-row items-center text-sm mb-4">
                        <h3 className="bg-yellow-300 text-black rounded-full p-1 mr-2">Cat</h3>
                        <h3 className="bg-yellow-300 text-black rounded-full p-1 mr-2">Dog</h3>
                    </div>
                    ...
                    <button className="absolute bottom-4 right-8 rounded-xl bg-gray-700 py-2 px-4" onClick={() => handleDownloadClick(data.datasetUrl)}>Download</button>
                </div>
                ...
                <div className="w-4/5 h-auto flex-shrink-0">
                    ...
                    <div className="w-full h-auto mt-4 flex flex-row">
                        ...
                        <div className="ml-8 h-auto w-full border border-white py-1 px-4">
                            ...
                            <textarea className="font-light bg-transparent focus:outline-none w-full resize-none" value={commentBody} placeholder="Your comment" onChange={(e) => handleCommentChange(e)}/>
                            <div className="w-full flex flex-row justify-end">
                                <button className="rounded-xl bg-gray-700 py-1 px-2 m-1" onClick={() => postComment()}>Post</button>
                            </div>
                        </div>
                    </div>
